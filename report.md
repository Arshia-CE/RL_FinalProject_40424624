...
![MazeMario title menu](results/figures/gui/gui_title.png)

The title/pause screen carries the first two spec controls as game menus:
world selection (source + both transfer targets — the exact committed
maps), brain selection, and the mode toggle. Two design decisions are
visible: VALUE ITERATION is labeled *WATCH ONLY* and the TRAIN row is
grayed out while it is selected — VI is planned exactly from the model, so
there is nothing to watch train (§10, Q3) — and every selector edits in
place rather than rebuilding the menu, so switching choices is
flicker-free.
...
![MazeMario gameplay with the policy overlay](results/figures/gui/gui_gameplay.png)

Mid-episode WATCH gameplay (Value Iteration, World 1) with the POLICY
overlay on. The arrows are §3's optimal policy rendered live *for the
hero's current key status and gate phase* — they visibly reorganize as
the dragon cycle advances, which is the phase-dependence of §3's policy
grid made interactive. The moment captured shows the feature working:
the HUD reads DRAGON OUT·1 and the dragon stands in the doorway ahead of
the door, exactly the blocked corridor the timing analysis is about; the
key, a thorn pit and the princess are each visually distinct, as the
spec requires, and the HUD tracks score (−22 after 17 steps of −1s plus
one slip into a wall) and the step cap live.

![MazeMario victory screen](results/figures/gui/gui_course_clear.png)

Episode ends are unmissable, as the spec demands: reaching the princess
raises the COURSE CLEAR banner (a TIME UP variant covers step-cap
truncation). The numbers agree across the two displays — the overlay's
SCORE 204 · 39 STEPS is the HUD's final state, with KEY GOT! and WIN 100%
— because both read the same episode state; this particular run beat the
43.4-step evaluation mean of §3 by catching favorable slips.
...
(60 tests). The GUI screenshots in §8 are captures of the live
application and are the one visual artifact not generated by the
experiment pipeline.

## 13. Sibling implementations

This maze exists in four complete implementations, one per branch, each a
full re-run of the identical pipeline on byte-identical maps, and each
sibling's report closes with a retrospective measured against this
baseline:

| branch | dynamic feature | headline result |
|---|---|---|
| `main` (this) | periodic gate, **departure** semantics | the canonical baseline: every learner converges comfortably (Q-Learning ≈1,099 / SARSA(0.7) ≈982 episodes to 90%) |
| `future_phase` | same gate, open **on arrival** | a one-predicate change deepens the task at zero cost: the naive "enter iff it looks open" rule becomes provably suboptimal at 2 of 6 phases, and perfect play must budget ~3 steps of timing |
| `wizard_obstacle` | **moving obstacle** — a wizard blinking along a 4-cell doorway patrol | measured equivalence to the gate (V\*, sweeps, learner speeds all within noise) — and its knife-edge: equivalence holds only when the obstacle's *longest consecutive hold* matches the gate's closed interval; also contains the cross-design robustness audit separating method properties from dynamics accidents |
| `limited_energy` | observable **energy budget** (state ×17) | the deepest study: sparse one-step Q-Learning scales ×28.5 in episodes where traces scale ×11.5 and shaping ×14.8; an inverted "desperation" band (caution when poor); a permanent negative-transfer trap that only selective transfer survives; a measured E₀ operating curve |

The gate on this branch remains the cleanest minimal exhibit of temporal
Markov structure and the safest artifact set to reproduce; the deepest
analysis of the four lives on `limited_energy`, and the strongest
methodological evidence — which findings replicate across all dynamics
and which flip — lives in the `wizard_obstacle` retrospective. Read this
report for the canonical study, then those two for what the design space
